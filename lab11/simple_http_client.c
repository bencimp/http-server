...
int write_http_request(int sock_fd, const char *file) {
    ...
    char protver[] = " HTTP/1.1\r\n";

    char endreq[] = "\r\n";
    strcat(mybuffer, myget);
    strcat(mybuffer, file);
    strcat(mybuffer, protver);
    strcat(mybuffer, "Host: ");
    strcat(mybuffer, HOST);
    strcat(mybuffer, "\r\n");
    //strcat(mybuffer, "Accept: ");
    strcat(mybuffer, endreq);
    //strcat(mybuffer, myhost);

    printf("%s\n", mybuffer);

    //char myaccept[] = "Accept: ";

    //char myacceptlang[] = "Accept-Language: en-US\r\n";

    write(sock_fd, mybuffer, sizeof(mybuffer));
